...
parser.add_argument('--addlayer', '-a', action='store_true',
...
dsets = {
    x: ImageCSV('{}.csv'.format(x), x, data_transforms[x])
...
if (args.testOnly):
  ...
  testsets = ImageCSV(cf.test_dir, data_transforms['valid'])
  ...
  for batch_idx, (inputs, targets) in enumerate(testloader):  # dset_loaders['valid']):
    if use_gpu:
...
if __name__ == "__main__":
  ...
  model_ft = train_model(model_ft, criterion, optimizer_ft,
                         exp_lr_scheduler, num_epochs=cf.num_epochs)
